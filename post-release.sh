...
if [[ $(echo $SECOND_TO_LAST_COMMIT_MSG | grep '^Merge .*alpha') ]]; then
  echo '[tawfeer-scripts] Release was created from the alpha branch. Alpha branch will now be reset.'
  ...
  echo '[tawfeer-scripts] Release was created from a different branch than the alpha branch (e.g. a hotfix branch).'
  echo '[tawfeer-scripts] Alpha branch will now be updated with the lastest changes from release.'
  ...
  if [[ $? == 0 ]]; then
    ...
    git merge --abort
    echo '[tawfeer-scripts] Post-release merge to alpha failed.'
    if [ -z "$SLACK_CHANNEL_ID" ] || [ -z "$SLACK_AUTH_TOKEN" ]; then
      echo '[tawfeer-scripts] Missing Slack channel ID and/or token. Cannot notify.'
    else
      echo '[tawfeer-scripts] Notifying the team on Slack.'
      curl \
...
echo '[tawfeer-scripts] Merging the release branch into trunk.'
git checkout trunk

# Merge release branch into trunk branch, and notify the team if any conflicts arise.
git merge --no-ff release -m "chore(release): merge in release $LATEST_VERSION_TAG"
if [[ $? == 0 ]]; then
  echo '[tawfeer-scripts] Pushing updated trunk to origin.'
  git push "https://$GITHUB_TOKEN@github.com/$CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME.git"
else
  git merge --abort
  echo '[tawfeer-scripts] Post-release merge to trunk failed.'
  if [ -z "$SLACK_CHANNEL_ID" ] || [ -z "$SLACK_AUTH_TOKEN" ]; then
    echo '[tawfeer-scripts] Missing Slack channel ID and/or token. Cannot notify.'
  else
    echo '[tawfeer-scripts] Notifying the team on Slack.'
